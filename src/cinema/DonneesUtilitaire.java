...
public class DonneesUtilitaire {
    ...
    private static final String DB_FILE = "src/films.db";
    
    
    /**
     * Met à jour le fichier de sérialisation
     * 
     * @return
     */
	...
	try {
	    ...
	    oos.writeObject(ff.getActeurs());
	    oos.writeObject(ff.getFilms());
	} catch (Exception e) {
	...
	if (file.exists()) {
		...
		ff.setActeurs((HashMap<String, Acteur>) ois.readObject());
		ff.setFilms((HashMap<String, Film>) ois.readObject());
		
	    } catch (Exception e) {
		System.out.println("LOAD " + e);
		System.out.println("Try to delete src/films.db");
		success = false;
	    } finally {
		if (ois != null) {
		    try {
